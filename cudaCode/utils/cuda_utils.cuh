...
inline void createLevelDataOffset(cliqueLevelDataPointer levelData, ui offsetPartitionSize, ui totalWarps) {
    ...
    thrust::inclusive_scan(thrust::device, levelData.temp, levelData.temp + totalWarps + 1, levelData.temp);

    // Perform inclusive scan on count array
    thrust::inclusive_scan(thrust::device, levelData.count, levelData.count + totalWarps + 1, levelData.count);
}
